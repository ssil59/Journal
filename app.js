...
app.get("/", function(request, response){
  response.render("home", {homeContent : homeStartingContent, posts : posts});
});
...
app.get("/posts/:topic", function(request, response){
  ...
  let topicTitle = request.params.topic;
  posts.forEach(function(item){
    if(lodash.lowerCase(item.title) === lodash.lowerCase(topicTitle)){
      response.render("post", {postTitle : item.title, postBody : item.body});
    }
  });
